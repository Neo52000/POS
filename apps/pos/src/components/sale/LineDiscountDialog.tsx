...
export function LineDiscountDialog({
  ...
  const percentValid =
    percent !== '' && Number.isFinite(percentValue) && percentValue >= 0 && percentValue <= 100;
  const priceCents = parseEuroToCents(price);

  const applyPercent = (): void => {
    if (!line || !percentValid) return;
    onDiscount(line.key, Math.round(percentValue * 100) / 100);
    onClose();
  };

  const applyPrice = (): void => {
    if (!line || priceCents === null || priceCents < 0) return;
        ...
        <Tabs defaultValue="percent">
          ...
          <TabsContent value="price" className="flex flex-col gap-3">
            ...
            <DialogFooter>
              ...
              <Button size="touch" disabled={priceCents === null} onClick={applyPrice}>
                Forcer le prix
              </Button>
            </DialogFooter>
          </TabsContent>
